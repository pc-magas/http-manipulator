...
test("parse Multipart File",(done)=>{
    ...
    const fileContent = fs.readFileSync( path.resolve(fileDataPath,"img.png.base64")).toString();

    const detectFields = (err,fieldName,value,isFile,filename,detectedMime,inputMime,flags) => {
        ...
        if(fieldName=='key1'){
            ...
            expect(flags).toContain(FLAG_MIME_TYPE_MISMATCH)
        }
